...
public class CondoListDiff extends GetListsResponseDiff {
	
	private static final Integer DATA_INDEX = 2;
	
	public CondoListDiff () {
		super(DATA_INDEX);
	}
	...
	protected boolean diffData (XmlRpcArray newArray, XmlRpcArray oldArray) {
		...
		if (!diffs.isEmpty()) {
			areDifferent = true;
			addDiffMessage("ERROR: New condos '${diffs}' were not found in old feed!");
		}
		diffs = difference(oldData, newData)
		if (!diffs.isEmpty()) {
			areDifferent = true;
			addDiffMessage("ERROR: Old condos '${diffs}' were not found in new feed!");
		}
		return areDifferent;
	}
